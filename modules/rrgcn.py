    ...
    def predict(self, test_graph, test_triplets, use_cuda, neg_samples_batch=None, pos_samples_batch=None, 
        ...
        with torch.no_grad():
            ...
            if neg_samples_batch != None: # added by tgb team
                perf_list = []
                hits_list = []
                # all2 = [i for i in range(self.num_ents)]
                batch_size_total = len(neg_samples_batch)
                # scores = torch.zeros(len(neg_samples_batch), self.num_ents).cuda() if use_cuda else torch.zeros(len(neg_samples_batch), self.num_ents)
                scores_all = torch.zeros(len(neg_samples_batch), self.num_ents).cuda() if use_cuda else torch.zeros(len(neg_samples_batch), self.num_ents)
                for query_id, query in enumerate(neg_samples_batch): # for each sample separately
                    ...
                    score_list = self.decoder_ob.forward(evolve_embeddings, r_emb, test_triplets[query_id].unsqueeze(0),
                            samples_of_interest_emb= [evolve_embeddings[i]for i in range(len(evolve_embeddings))], batch_size_total=batch_size_total )
                    ...
                    scores_b_pos = scores_b[0][pos]
                    scores_b_neg = scores_b[0][neg]

                    # compute MRR
                    input_dict = {
                        "y_pred_pos": np.array([scores_b_pos.cpu()]),
                        "y_pred_neg": np.array([scores_b_neg.cpu()]),
                        "eval_metric": [metric],
                    }
                    prediction_perf = evaluator.eval(input_dict)
                    perf_list.append(prediction_perf[metric])
                    hits_list.append(prediction_perf['hits@10'])
                    # scores[query_id] = scores_b[0]
                    scores_all[query_id] = scores_b[0]
            ...
            return scores, perf_list, hits_list, scores_all
        ...
        with torch.no_grad():
            ...
            if neg_samples_batch != None: # added by tgb team
                ...
                for query_id, query in enumerate(neg_samples_batch): # for each sample separately
                    ...
                    score = self.decoder_ob.forward(embedding, r_emb, test_triplets[query_id].unsqueeze(0),
                            samples_of_interest_emb=embedding, batch_size_total=batch_size_total )
                    score_pos = np.array([score[0][pos].cpu()])
                    score_neg = np.array([score[0][neg].cpu()])
                    # compute MRR
                    input_dict = {
                        "y_pred_pos": score_pos, #np.array([score[0,0].cpu()]),
                        "y_pred_neg": score_neg, #np.array(score[0,1:].cpu()),
                        "eval_metric": [metric],
                    }
                    prediction_perf = evaluator.eval(input_dict)
                    perf_list.append(prediction_perf[metric])
                    hits_list.append(prediction_perf['hits@10'])
                    scores[query_id] = score[0]
            else:
            ...
            return score, perf_list, hits_list, scores
