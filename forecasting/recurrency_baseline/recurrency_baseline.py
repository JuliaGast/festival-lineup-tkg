...
def predict(num_processes,  data_c_rel, all_data_c_rel, alpha, lmbda_psi,
            perf_list_all, hits_list_all, window, neg_sampler, split_mode, write_rankings_flag=False, file=None):
    ...
    if num_processes > 1:
        print('Logging the rankings for the concert tkg only works with one process. Setting num_processes to 1 for logging the rankings.')
        perf_list, hits_list = baseline_predict(len(data_c_rel), data_c_rel, all_data_c_rel, 
                    window, basis_dict, 
                    num_nodes, num_rels, lmbda_psi, 
                    alpha, evaluator, first_ts, neg_sampler, split_mode,  file=file, write_rankings_flag=write_rankings_flag)      
        # object_references =[]                   
...
## test
def test(best_config, all_relations,test_data_prel, all_data_prel, neg_sampler, num_processes, window, split_mode='test', path_rankings='default.txt', write_rankings_flag=False):  
    ...
    if write_rankings_flag:    
        path_rankings_file = path_rankings + '_'+split_mode + '.txt'
    ...
    for rel in all_relations:
        ...
        if rel in test_data_prel.keys():
            ...
            perf_list_rel, hits_list_rel = predict(num_processes, test_data_c_rel,
                                                ...
                                                window, neg_sampler, split_mode, file=file, write_rankings_flag=write_rankings_flag)
...
## train
def train(params_dict, rels,val_data_prel, trainval_data_prel, neg_sampler, num_processes, window, path_rankings='default.txt'):
...
def get_args(): 
    ...
    parser.add_argument("--dataset", "-d", default="tkgl-concertperformanceonly", type=str) 
    ...
    parser.add_argument('--write_rankings_flag', type=bool, default=True)
    parser.add_argument("--rels_of_interest", type=int, nargs="+",  default=[0,1]) #, 23] ) #[0, 1]) # set to None if you want all rels. one or more values, e.g. --rels_of_interest 0 1 5
...
rels_of_interest = parsed['rels_of_interest'] # only look at these relations
...
if rels_of_interest:
    rankings_filename = 'rankings_'+str(parsed['window'])+'_'+str(parsed["train_flag"])+'_rels_'+str(rels_of_interest).replace('[','').replace(']', '').replace(', ', '_')
else:
    rankings_filename = 'rankings_'+str(parsed['window'])+'_'+str(parsed["train_flag"])+'_rels_all'
...
if parsed['train_flag'] ==  'True':
    if parsed['load_flag'] == 'True':
        with open(f'{osp.dirname(osp.abspath(__file__))}/best_config.json', 'r') as infile:
        ...
        if parsed['save_config'] == 'True':
            import json
            with open(f'{osp.dirname(osp.abspath(__file__))}/best_config.json', 'w') as outfile:
...
perf_list_all_val, hits_list_all_val = test(best_config,rels, val_data_prel, 
                                            ...
                                            parsed['window'], split_mode='val', write_rankings_flag=parsed['write_rankings_flag'], path_rankings=rankings_path)
...
perf_list_all, hits_list_all = test(best_config,rels, test_data_prel, 
                                                ...
                                                parsed['window'], write_rankings_flag=parsed['write_rankings_flag'], path_rankings=rankings_path)
