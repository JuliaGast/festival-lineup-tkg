...
class Trainer(object):
    ...
    def train_epoch(self, rankings_file=None):
        self.now_epoch+=1
        ...
        t_mrr, t_h1, t_h3, t_h10, t_h100,t_cover,t_loss = self.evaluate(data_eval="test", write_rankings_flag=True, rankings_file=rankings_file)
    ...
    def evaluate_with_single_steps(self, data_eval='valid', write_rankings_flag=False, rankings_file=None):
        ...
        if start_time_stamp >= end_time_stamp:
            raise Exception('Error!')
        return self.evaluate_base(start_time_stamp, end_time_stamp, get_evidence_start_point, write_rankings_flag=write_rankings_flag, file=rankings_file)

    def evaluate_base(self, start_time_stamp, end_time_stamp, get_evidence_start_point:Callable, write_rankings_flag=False, file=None):
        self.tred_gnn.eval()
        ranks = []
        counter = np.array([0.,0.,0.])
        results_dict_head ={}
        ...
        for time_stamp in tqdm(range(start_time_stamp, end_time_stamp), file=sys.stdout, disable=self.opts.disable_bar):
            ...
            for i in range(num_batch):
                ...
                if write_rankings_flag:
                    for scores_l, query_l in zip(scores, data_batched.tolist()):
                        query = (head_write, relh_write, t_write) = (query_l[0], query_l[1], int(time_stamp))
                        if not query in results_dict_head:
                            # file.write(str(head_write) + " " + str(relh_write) + " " + "?" + " " + str(t_write) + "\n")
                            results_dict_head[query] = {}
                            preds_sorted, indices_sorted = torch.sort(scores_l, descending=True)
    ...
    def evaluate_base(self, start_time_stamp, end_time_stamp, get_evidence_start_point:Callable, write_rankings_flag=False, file=None):
        ...
        for time_stamp in tqdm(range(start_time_stamp, end_time_stamp), file=sys.stdout, disable=self.opts.disable_bar):
            ...
            for i in range(num_batch):
                ...
                if write_rankings_flag:
                    for scores_l, query_l in zip(scores, data_batched.tolist()):
                        query = (head_write, relh_write, t_write) = (query_l[0], query_l[1], int(54)) # manually write the end time to 54 to be compatible with eval code
                        if not query in results_dict_head:
                            file.write(str(head_write) + " " + str(relh_write) + " " + "?" + " " + str(t_write) + "\n")
                            results_dict_head[query] = {}

                            indices_sorted =  np.argsort(scores_l)[::-1]       
                            preds_sorted = scores_l[indices_sorted]
