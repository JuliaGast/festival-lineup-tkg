            ...
            for t in test_queries_of_interest[rel][head]:
                ...
                if R > 0:
                    if len(rankings[(head, rel,t)]) == 0:
                        a =1
                    scores_array =rule_utils.create_scores_array(rankings[(head, rel,t)], num_nodes)
                    ...
                    # num_correct_in_top_R_ten_or_less = sum([1 for tail in top_TenOrLess_predictions if tail in ground_truth_tails])
...
if __name__ == "__main__":
    ...
    dataset_name = "tkgl-concert5"
    dataset_name_folder = dataset_name.replace("-", "_")
    rankings_name = "tkgl-concert520-rankings_test_conf_0_corr_conf_0_noisyor_crules_frules_zrules_pvalue_10_num_top_rules_5_multi.txt"

    rel_list = ['performs_at_festival']
    year = '2023'
    print(f"evaluating for relation {rel_list} and year {year} with ranking file {rankings_name} on dataset {dataset_name}")
