...
def csv_to_wikidata(
    ...
    """
    feat_size = 1
    num_lines = sum(1 for line in open(fname, encoding="utf-8")) - 1
    print("number of lines counted", num_lines)
    u_list = np.zeros(num_lines)
    i_list = np.zeros(num_lines)
    ts_list = np.zeros(num_lines)
    ...
    csv.field_size_limit(max_int) # add this line to avoid csv field size limit error for concert dataset, which has some very long text fields that we need to read in to get the node ids and relation types
    ...
    with open(fname, "r") as csv_file:
        ...
        for row in tqdm(csv_reader):
            if idx == 0:
                ...
                edge_type[idx - 1] = relation
                idx += 1
    if 'yago' in fname or 'icews14' in fname or 'icews18' in fname or 'wikiold' in fname or 'gdelt'  in fname or 'monkey' in fname  or 'pig' in fname  or 'crisis'  in fname or 'concert' in fname:
        input_file = 'entity2id.txt'
        input_file_rel = "relation2id.txt"
    elif 'muffi' in fname:
        input_file_rel = 'relation2id.txt'
    ...
    if input_file !=None:
        ...
        with open(input_dir, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                if len(parts) < 2:
                    continue
                if 'wikidata' in fname or 'smallpedia' in fname or 'concert' in fname:
    ...
    if input_file_rel !=None:
        ...
        with open(input_dir, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                if len(parts) < 2:
                    continue
                if 'wikidata' in fname or 'concert' in fname or 'smallpedia' in fname:
                    string, original_id = parts[1], parts[0]
                else:
                    string, original_id = parts[0], int(parts[1])
