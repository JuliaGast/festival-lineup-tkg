...
from forecasting.tgb.linkproppred.negative_sampler import NegativeEdgeSampler
from forecasting.tgb.linkproppred.tkg_negative_sampler import TKGNegativeEdgeSampler

from forecasting.tgb.utils.info import (
    PROJ_DIR, 
    DATA_URL_DICT, 
    DATA_VERSION_DICT, 
...
from forecasting.tgb.utils.pre_process import (
    csv_to_pd_data,
    process_node_feat,
    process_node_type,
    csv_to_pd_data_sc,
...
from forecasting.tgb.utils.utils import save_pkl, load_pkl
from forecasting.tgb.utils.utils import add_inverse_quadruples
...
class LinkPropPredDataset(object):
    ...
    def generate_processed_files(self) -> pd.DataFrame:
        ...
        if (osp.exists(OUT_DF)) and (self.version_passed is True):
            ...
            if self.name == "tgbl-flight":
                ...
                df, edge_feat, node_ids = csv_to_tkg_data(self.meta_dict["fname"])
            elif self.name == 'tkgl-concert':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])  
            elif self.name == 'tkgl-concertwithshortcuts':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])  
            elif self.name == 'tkgl-concertperformanceonly':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])
            elif self.name == 'tkgl-concertmini':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])
            elif self.name == 'tkgl-concert5':  
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])
            elif self.name == 'tkgl-concert6':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])
            elif self.name == 'tkgl-concert2023':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])
            elif self.name == 'tkgl-concert2024':
                df, edge_feat, node_ids = csv_to_wikidata(self.meta_dict["fname"])
    ...
    def pre_process(self):
        ...
        elif('concert' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=53, max_val_ts=53, min_test_ts=54, max_test_ts=54)
        elif('concertwithshortcuts' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=53, max_val_ts=53, min_test_ts=54, max_test_ts=54)
        elif('concertperformanceonly' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=53, max_val_ts=53, min_test_ts=54, max_test_ts=54)
        elif('concertmini' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=53, max_val_ts=53, min_test_ts=54, max_test_ts=54)
        elif('concert5' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=53, max_val_ts=53, min_test_ts=54, max_test_ts=54)
        elif('concert6' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=53, max_val_ts=53, min_test_ts=54, max_test_ts=54)
        elif('concert2024' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=52, max_val_ts=52, min_test_ts=53, max_test_ts=53)
        elif('concert2023' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits_fixed_ts(full_data, min_val_ts=51, max_val_ts=51, min_test_ts=52, max_test_ts=52)
        elif('concert' in self.name):
            _train_mask, _val_mask, _test_mask = self.generate_splits(full_data, val_ratio=0.061, test_ratio=0.06)
        else:
    ...
    def generate_splits_fixed_ts(
        ...
        Args:
            ...
            min_val_ts: minimum timestamp for validation data
            max_val_ts: maximum timestamp for validation data
            min_test_ts: minimum timestamp for test data
            max_test_ts: maximum timestamp for test data
        Returns:
        ...
        assert min_test_ts > max_val_ts, "min_test_ts should be greater than max_val_ts to avoid overlap between val and test sets"
        train_mask = timestamps < min_val_ts
        val_mask = np.logical_and(timestamps <= max_val_ts, timestamps >= min_val_ts)
        test_mask = np.logical_and(timestamps <= max_test_ts, timestamps >= min_test_ts)

        if len(timestamps[train_mask]) == 0:
            print(f"Warning: no training samples found with the given timestamp splits. Please check the timestamp distribution and the split timestamps. BUG?")
        if len(timestamps[val_mask]) == 0:
            print(f"Warning: no validation samples found with the given timestamp splits. Please check the timestamp distribution and the split timestamps. BUG?")
        if len(timestamps[test_mask]) == 0:
            print(f"Warning: no test samples found with the given timestamp splits. Please check the timestamp distribution and the split timestamps. BUG?")
