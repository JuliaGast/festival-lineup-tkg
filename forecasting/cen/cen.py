...
def test(model, history_len, history_list, test_list, num_rels, num_nodes, use_cuda, model_name, mode, split_mode, write_rankings_flag=False, file=None):
    """
    Test the model
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        _, perf_list, hits_list, predictions_all = model.predict(history_glist, test_triples_input, use_cuda, neg_samples_batch, pos_samples_batch, 
                                    evaluator, METRIC) 
...
def run_experiment(args, trainvalidtest_id=0, n_hidden=None, n_layers=None, dropout=None, n_bases=None, rankings_file=None):
    '''
    ...
    if trainvalidtest_id == 1:  # normal test on validation set  Note that mode=test
        ...
        if os.path.exists(test_state_file):
            mrr, perf_per_rel, hits10 = test(model, args.test_history_len, train_list+valid_list, test_list, num_rels, num_nodes, use_cuda, 
                        test_state_file, "test", split_mode="test", write_rankings_flag=True, file=rankings_file)
        else:
            print('Cannot do testing because model does not exist: ', test_state_file)
            mrr = 0
            hits10 = 0
...
path_rankings_file = rankings_path + '_'+'cen_test' + '.txt'
...
if args.grid_search:
    ...
    best_mrr = 0
    best_config = {}
    for start_history_len in [1,2,3]: 
        for n_layers in [1, 2]:
            for dropout in [0.2, 0.4]:
                # train-history-len, and  -test-history-len
                start_train = timeit.default_timer()
                args.start_history_len = start_history_len
                args.dropout = dropout
                args.n_layers = n_layers


                val_mrr, _, _, hits10 = run_experiment(args, trainvalidtest_id=-1)
                # train
                val_mrr, args.test_history_len, _, hits10 = run_experiment(args, trainvalidtest_id=0) # overwrite test_history_len with 
                # the best history len (for valid mrr)  
                if val_mrr > best_mrr:
                    best_mrr = val_mrr
                    best_config = {
                        'start_history_len': start_history_len,
                        'dropout': dropout,
                        'n_layers': n_layers

                    }
                    print(f"New best config found: {best_config} with val MRR: {best_mrr: .4f} ")
                else:
                    print(f"Config: start_history_len: {start_history_len},  has val MRR: {val_mrr: .4f}. this is not better than current best MRR: {best_mrr: .4f} with config {best_config}")
                one_time = timeit.default_timer() - start_train

    # one last time with best config
    start_train = timeit.default_timer()

    args.start_history_len = best_config['start_history_len']
    ...
    if write_rankings_flag:    
        ...
        test_file = file
    print('start testing')
    test_mrr, perf_per_rel, test_hits10 = run_experiment(args, rankings_file=test_file) # do testing
...
#single run
    ...
    # test on test set
    start_test = timeit.default_timer()
    test_mrr, _, perf_per_rel, test_hits10 = run_experiment(args, trainvalidtest_id=2, rankings_file=rankings_file)
