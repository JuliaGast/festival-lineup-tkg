...
def evaluate(rule_dataset, path_rankings, progressbar_percentage, evaluation_mode="test", eval_type='average', special_evalquads=None, rels_of_interest=None):
    ...
    with tqdm(total=total_iterations) as pbar:
        ...
        for i, (src, dst, t, rel) in enumerate(zip(testdata[:,0], testdata[:,2], testdata[:,3], testdata[:,1])):
            ...
            scores_array, _ =utils.create_scores_array(rankings[(src, rel,t)], num_nodes)
    ...
    for rel in mrr_per_rel:
        print(f"rel {rel}: mrr {mrr_per_rel[rel][0]:.4f} over {mrr_per_rel[rel][1]} samples")
        print(f"rel {rel}: hits@1 {hit1_per_rel[rel][0]:.4f} over {hit1_per_rel[rel][1]} samples")
...
if __name__ == "__main__":
    dataset_name = "tkgl-concert2"
    ruledataset = RuleDataset(name=dataset_name, large_data_hardcode_flag=False)

    path_rankings = osp.join(osp.dirname(__file__), "..", "files", "rankings", dataset_name, "ranking-both-all-mixtral-8x7bt2025.txt")
    print('path_rankings:', path_rankings)


    scores = evaluate(ruledataset, path_rankings, progressbar_percentage=0.01, evaluation_mode='test', eval_type='random', rels_of_interest=[0,1])
